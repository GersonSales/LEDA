...
public class BSTVerifierImplTest {
	...
	public void test4() {
		...
		avl.getRoot().getRight().getRight().getRight().getRight().setData(5);
		avl.getRoot().getRight().getRight().getRight().getRight().setLeft(getNilNode());
		avl.getRoot().getRight().getRight().getRight().getRight().setRight(getNilNode());

		Assert.assertTrue(avlVerifier.isBST());
		Assert.assertFalse(avlVerifier.isAVLTree());

	}

	@Test
	public void test5() {
		avlVerifier = new AVLTreeVerifierImpl<>(null);
		bstVerifier = new BSTVerifierImpl<>(null);
