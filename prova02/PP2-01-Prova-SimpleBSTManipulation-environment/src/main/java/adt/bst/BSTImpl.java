...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	public int[] nodesByLevel() {
		...
		Queue<BTNode<T>> polledNodeQueue = new LinkedList<>();

		int[] counting = new int[height() + 1];
		nodeQueue.add(getRoot());

		for (int i = 0; i <= height(); i++) {

			while (!nodeQueue.isEmpty()) {
				BTNode<T> polledNode = nodeQueue.poll();
				if (!polledNode.isEmpty()) {
					polledNodeQueue.add(polledNode);
					counting[i]++;
				}
			}

			while (!polledNodeQueue.isEmpty()) {
				BTNode<T> polledNode = polledNodeQueue.poll();
				BTNode<T> polledLeftNode = polledNode.getLeft();
				BTNode<T> polledRightNode = polledNode.getRight();
				if (!polledLeftNode.isEmpty()) {
					nodeQueue.add(polledLeftNode);
				}

				if (!polledRightNode.isEmpty()) {
					nodeQueue.add(polledRightNode);
				}
			}
		...
		if (element != null) {
			...
			if (!node.isEmpty()) {
				if (!node.getRight().isEmpty()) {
					...
					return (BSTNode<T>) getClosestLeftParent(node, node.getParent());
				}
			}
		}
		return null;
		...
		if (element != null) {
			...
			if (!node.isEmpty()) {
				if (!node.getLeft().isEmpty()) {
					...
					return (BSTNode<T>) getClosestRightParent(node, node.getParent());
				}
			}
		}
		return null;
	...
	private void deleteWithSuccessor(BTNode<T> node) {
		BTNode<T> successor = sucessor(node.getData());
	...
	private int numberOfGradeOne(BTNode<T> node) {
		if (node != null) {
			if (isOlnyChildParent(node)) {
				return 1 + numberOfGradeOne(node.getLeft()) + numberOfGradeOne(node.getRight());
			} else {
				return numberOfGradeOne(node.getLeft()) + numberOfGradeOne(node.getRight());
			}
		}
		return 0;
	}
	...
	private int numberOfGradeTwo(BTNode<T> node) {
		if (node != null) {
			if (isTwoChildrenParent(node)) {
				return 1 + numberOfGradeTwo(node.getLeft()) + numberOfGradeTwo(node.getRight());
			}
		}
		return 0;
	}
