...
public class MinHeapImpl<T extends Comparable<T>> implements MinHeap<T> {
	...
	public void insert(T element) {
		...
		// Se o heap estiver cheio, aumenta o tamanho de acordo com o fator de aumento
		if (size >= heap.length) heap = Arrays.copyOf(heap, heap.length + INCREASING_FACTOR);
	...
	public T[] heapsort(T[] array) {
		...
		// Enquanto o heap nao estiver vazio ele vai extraindo os elementos pela raiz
		while(!isEmpty()) result.add(extractRootElement());
	...
	public void buildHeap(T[] array) {
		...
		for (int i = 0; i < array.length; i++) heap[i] = array[i];
		
		//Executa o heapify nos nós de baixo para cima ate chegar no nó
		for (int i = ((array.length - 1) / 2); i >= 0; i--) heapify(i);
	...
	private void heapify(int position){
		...
		if ((left <= size-1)) {
			...
			if (heap[left].compareTo(heap[position]) < 0) largest = left;
		...
		if ((right <= size-1)) {
			...
			if (heap[right].compareTo(heap[largest]) < 0) largest = right;
			
		}
		
		// Se o novo indice encontrado for diferente do indice informado eh por que ainda nao terminou a organizacao
		if (largest != position){ 
	...
	public T[] toArray() {
		...
		for(int i = 0; i < result.length; i++) result[i] = heap[i];
